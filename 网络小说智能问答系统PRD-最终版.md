...
**检索机制**（Hybrid Search + 智能路由 + 查询优化）：

1. **查询理解与改写**（阶段1）：
   - **智能查询改写**：使用LLM优化查询表述，提升检索召回率
   - **实体提取**：HanLP专业中文NER识别角色、地点、组织
   - **查询分类**：自动识别对话/分析/事实类查询
   - 查询向量化（智谱AI Embedding-3）

2. **双路检索**（阶段2 - GraphRAG）：
   - **语义检索**：
     - ChromaDB向量相似度搜索（L2距离 + 高斯核转换）
     - Top-K: 30个候选块（增加召回）
   - **图谱检索**：
     - 基于实体查询知识图谱关系
     - 动态计算章节重要性（新增实体、关系变化、事件密度）
   - **关键词检索**：元数据精确匹配 + 章节范围过滤
...
4. **动态Rerank**（阶段3）：
   - **实体感知排序**：优先匹配查询实体的文档
   - **动态权重调整**：
     - **事实类**：高相似度时增强语义权重，低相似度时增强实体匹配
     - **对话类**：高相似度时降低对话标记影响
     - **分析类**：低相似度时增强章节重要性
   - **时间衰减可选**：用户可配置后期章节权重偏向（0.0-0.5，默认0.15）
   - 最终返回Top-10块

5. **自适应Prompt构建**（阶段4）：
   - 根据查询类型选择专用Prompt模板
   - **Few-shot示例**：提供典型问答示例引导回答
   - **CoT引导**：分析类查询添加思维链提示
...
#### 4.4.2 查询处理流程（增强版 GraphRAG + Self-RAG）

**核心步骤**（已实现所有查询优化）：

1. **查询理解与改写**：
   - **智能查询改写**：使用LLM（GLM-4）优化用户查询，提升检索效果
   - **实体提取**：HanLP专业中文NER提取角色、地点、组织
   - **查询分类**：自动识别对话/分析/事实类查询
   - **向量化**：智谱Embedding-3生成查询向量

2. **双路检索（GraphRAG）**：
   - **语义检索**：
     - ChromaDB向量相似度搜索（L2距离度量）
     - 使用高斯核函数转换距离为相似度
     - Top-30候选块（提升召回率）
   - **图谱检索**：
     - 基于实体查询NetworkX知识图谱
     - 获取角色关系、时序信息
     - 动态计算章节重要性（新增实体30% + 关系变化50% + 事件密度20%）
...
4. **实体感知Rerank**：
   - **实体匹配评分**：计算文档与查询实体的匹配度
   - **动态权重调整**：
     - 事实类：base_score高时增强语义权重1.2倍，entity_match低时降低实体权重
     - 对话类：base_score>0.85时降低quote_boost影响50%
     - 分析类：base_score<0.60时增强章节重要性30%
   - **时间衰减（可选）**：用户可配置后期章节权重偏向（0.0-0.5）
   - 选取Top-10块

5. **自适应Prompt构建**：
   - 根据查询类型选择专用Prompt模板
   - **Few-shot示例**：内置典型问答示例引导生成
   - **CoT引导**：分析类查询添加思维链提示
   - 将检索结果和图谱信息组装为完整上下文
...
<SettingsPage>
  <SettingsSections>
    ...
    <Section title="🔧 高级设置">
      ...
      <FormItem label="启用查询改写">
        <Switch checked={enableQueryRewrite} defaultChecked={true} />
        <HelpText>使用LLM优化查询表述，提升检索效果</HelpText>
      </FormItem>
      <FormItem label="时间衰减权重">
        <Slider min={0} max={0.5} step={0.05} value={recencyBiasWeight} defaultValue={0.15} />
        <HelpText>后期章节权重偏向（0.0=无偏向, 0.5=强偏向）</HelpText>
      </FormItem>
    </Section>
  </SettingsSections>
</SettingsPage>
...
#### 2. 查询理解增强
- **智能查询改写**：使用LLM自动优化查询表述，提升检索召回率
- **查询类型分类**：LLM自动识别对话/分析/事实类查询
- **HanLP实体提取**：专业中文NER识别角色、地点、组织
- **前端可见改写**：改写后查询显示在前端，用户可见优化效果

#### 3. 检索机制全面升级
- **相似度算法切换**：从Cosine距离切换至**L2距离（欧氏距离）**
- **高斯核转换**：使用exp(-distance²/2)将L2距离转换为相似度得分
- **实体感知排序**：计算文档与查询实体的匹配度，优先匹配实体的文档
- **动态权重调整**：
  - **事实类**：高相似度时增强语义权重1.2倍，低匹配度时调整实体权重
  - **对话类**：高相似度时降低对话标记影响50%
  - **分析类**：低相似度时增强章节重要性30%
- **候选数增加**：从Top-20提升至Top-30，提高召回率

#### 4. 自适应Prompt技术
- **查询类型专用模板**：对话/分析/事实类使用不同Prompt模板
- **Few-shot示例引导**：内置典型问答示例，引导生成高质量答案
- **Chain of Thought（CoT）**：分析类查询添加思维链提示，引导逐步推理

#### 5. 时间衰减可配置
- **后期章节偏向**：用户可配置时间衰减权重（0.0-0.5）
- **指数增长计算**：使用exp(bias_weight × position)计算衰减得分
- **前端UI控制**：滑块调节，实时生效，适应不同查询偏好
- **默认温和设置**：默认0.15，平衡前后期内容

#### 6. GraphRAG增强
- **置信度评分**：根据图谱一致性和向量匹配度综合打分
- **章节重要性动态评分**：基于新增实体30% + 关系变化50% + 事件密度20%
...
#### 8. 开发者友好特性
- **TRACE级别日志**：自定义日志级别，完整记录每步输入输出
- **独立日志文件**：query_detail.log独立记录查询详情
- **Query ID追踪**：每次查询分配唯一ID，方便调试追踪
- **长文本可读**：日志中长文本完整输出，配合emoji标识不同步骤
